...
async function getTickets() {
  try {
    ...
    const header = ticketsContainer.querySelector(
      `[data-sort="${currentTicketsSortBy}"]`
    )!
...
ticketsContainer.addEventListener('click', (event) => {
  ...
  if (header) {
    ...
    currentTicketsPage =
      parseInt(header.getAttribute('data-page') as string) ?? currentTicketsPage
    currentTicketsLimit =
      parseInt(header.getAttribute('data-limit') as string) ??
      currentTicketsLimit
    currentTicketsSortBy =
      header.getAttribute('data-sort') ?? currentTicketsSortBy
