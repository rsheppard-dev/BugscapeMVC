...
async function getProjects() {
  try {
    ...
    const header = projectsContainer.querySelector(
      `[data-sort="${currentProjectsSortBy}"]`
    )!
...
projectsContainer.addEventListener('click', (event) => {
  ...
  if (header) {
    ...
    currentProjectsPage =
      parseInt(header.getAttribute('data-page') as string) ??
      currentProjectsPage
    currentProjectsSortBy =
      header.getAttribute('data-sort') ?? currentProjectsSortBy
